# Code taken from https://github.com/827916600/ClusterEnsembles/blob/main/ClusterEnsembles/ClusterEnsembles.py
# ClusterEnsembles.py
...
def to_pymetis_format(adj_mat):
    ...
    for i in range(n_rows):
        ...
        adjncy += list(idx_col)
        eweights += list(np.asarray(val).reshape(-1))
...
def cspa(base_clusters, nclass):
    ...
    membership = pymetis.part_graph(
        nparts=nclass, xadj=xadj, adjncy=adjncy, eweights=eweights
    )[1]
...
def mcla(base_clusters, nclass, random_state):
    ...
    meta_clusters = sparse.dok_matrix(
        (base_clusters.shape[1], nclass), dtype=float
    ).tolil()
...
def hbgf(base_clusters, nclass):
    ...
    rowA, colA = A.shape
    W = sparse.bmat(
        [[sparse.dok_matrix((colA, colA)), A.T], [A, sparse.dok_matrix((rowA, rowA))]]
    )
    xadj, adjncy, _ = to_pymetis_format(W)
    membership = pymetis.part_graph(
        nparts=nclass, xadj=xadj, adjncy=adjncy, eweights=None
    )[1]
...
def cluster_ensembles_package_run(
    ...
    if not ((random_state is None) or isinstance(random_state, int)):
        raise ValueError(
            f"Number of random_state must be a nonnegative integer; got (random_state={random_state})"
        )
    ...
    if solver == "cspa":
        if base_clusters.shape[1] > 10000:
            raise ValueError(
        ...
        raise ValueError(
            f"Invalid solver parameter: got '{solver}' instead of one of ('cspa', 'mcla', 'hbgf', 'nmf', 'all')"
        )
