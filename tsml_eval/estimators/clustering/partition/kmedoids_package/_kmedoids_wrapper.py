...
class DynkResult:
    ...
    def __init__(
        self, loss, labels, medoids, bestk, losses, rangek, n_iter=None, n_swap=None
    ):
        self.loss = loss
        self.labels = labels
        self.medoids = medoids
...
def _check_medoids(diss, medoids, init, random_state):
    ...
    if isinstance(medoids, int):
        ...
        if not isinstance(random_state, np.random.RandomState):
            raise ValueError(
                "Pass a numpy random generator, RandomState or integer seed"
            )
        return random_state.choice(diss.shape[0], medoids, False).astype(np.uintp)
    raise ValueError(
        "Specify the number of medoids, or give a numpy array of initial medoids"
    )
...
def fasterpam(diss, medoids, max_iter=100, init="random", random_state=None, n_cpu=-1):
    ...
    if isinstance(diss, np.ndarray):
        dtype = diss.dtype
        if n_cpu == -1 and diss.shape[0] < 1000:
            n_cpu = 1
        if n_cpu == -1 and os.cpu_count() is not None:
            n_cpu = os.cpu_count()
        if n_cpu == -1:
            n_cpu = 1
        assert n_cpu > 0
        if n_cpu > 1:
            ...
            if dtype == np.float32:
                return KMedoidsResult(
                    *_par_fasterpam_f32(diss, medoids, max_iter, seed, n_cpu)
                )
            elif dtype == np.float64:
                return KMedoidsResult(
                    *_par_fasterpam_f64(diss, medoids, max_iter, seed, n_cpu)
                )
            elif dtype == np.int32:
                return KMedoidsResult(
                    *_par_fasterpam_i32(diss, medoids, max_iter, seed, n_cpu)
                )
            elif dtype == np.int64:
                return KMedoidsResult(
                    *_par_fasterpam_i64(diss, medoids, max_iter, seed, n_cpu)
                )
            ...
            if dtype == np.float32:
                return KMedoidsResult(
                    *_rand_fasterpam_f32(diss, medoids, max_iter, seed)
                )
            elif dtype == np.float64:
                return KMedoidsResult(
                    *_rand_fasterpam_f64(diss, medoids, max_iter, seed)
                )
            elif dtype == np.int32:
                return KMedoidsResult(
                    *_rand_fasterpam_i32(diss, medoids, max_iter, seed)
                )
            elif dtype == np.int64:
                return KMedoidsResult(
                    *_rand_fasterpam_i64(diss, medoids, max_iter, seed)
                )
...
def pammedsil(diss, medoids, max_iter=100, init="build", random_state=None):
    ...
    """
    import numpy as np
    from kmedoids.kmedoids import _pammedsil_swap_f32, _pammedsil_swap_f64
...
def pamsil(diss, medoids, max_iter=100, init="build", random_state=None):
    ...
    """
    import numpy as np
    from kmedoids.kmedoids import _pamsil_swap_f32, _pamsil_swap_f64
...
def fastmsc(diss, medoids, max_iter=100, init="random", random_state=None):
    ...
    """
    import numpy as np
    from kmedoids.kmedoids import _fastmsc_f32, _fastmsc_f64
...
def fastermsc(diss, medoids, max_iter=100, init="random", random_state=None):
    ...
    """
    import numpy as np
    from kmedoids.kmedoids import _fastermsc_f32, _fastermsc_f64
...
def dynmsc(diss, medoids, minimum_k=2, max_iter=100, init="random", random_state=None):
    ...
    """
    import numpy as np
    from kmedoids.kmedoids import _dynmsc_f32, _dynmsc_f64
...
def silhouette(diss, labels, samples=False, n_cpu=-1):
    ...
    if not isinstance(diss, np.ndarray):
        diss = np.array(diss)
    labels = np.unique(labels, return_inverse=True)[1].astype(
        np.uintp
    )  # ensure labels are 0..k-1

    if isinstance(diss, np.ndarray):
        dtype = diss.dtype
        if n_cpu == -1 and samples:
            n_cpu = 1
        if n_cpu == -1:
        ...
        if n_cpu > 1:
            ...
            if dtype == np.float32:
                ...
                raise ValueError(
                    "Input of int64 is currently not supported, as it could overflow the float64 used internally when computing Silhouette. Use diss.astype(numpy.float64) if that is acceptable and you have the necessary memory for this copy."
                )
        else:
            if dtype == np.float32:
...
def medoid_silhouette(diss, meds, samples=False):
    ...
    import numpy as np
    from kmedoids.kmedoids import (
        _medoid_silhouette_f32,
        _medoid_silhouette_f64,
        _medoid_silhouette_i32,
    )
...
class KMedoids(SKLearnClusterer):
    ...
    def fit(self, X, y=None):
        ...
        medoids_params = self.n_clusters
        if self.initial_medoids is not None:
            medoids_params = self.initial_medoids
    ...
    def predict(self, X):
        ...
        if self.metric != "precomputed":
            ...
            raise NotImplementedError(
                "This API is not safe to use with precomputed distances. Use the argmin of the distances to the medoids."
            )
    ...
    def transform(self, X):
        ...
        if self.metric == "precomputed":
            ...
            return pairwise_distances(
                X, Y=Y, metric=self.metric, metric_params=self.metric_params
            )
