...
class USSL(BaseClusterer):
    ...
    def _fit(self, X, y=None):
        ...
        cluster_centres, distance_matrix_to_centres = self._ussl_weird_kmeans(
            distance_matrix
        )
        ...
        while gap > self.epsilon:
            # Calculation matrix
            X_tp1, Xkj_tp1_skl = self._distance_timeseries_shapelet(
                X, S_tp1
            )  # update X_tp1
            L_G_tp1, G_tp1 = self.spectral_timeseries_similarity(
                X_tp1
            )  # update L_G_tp1
            SS_tp1, XS_tp1, SSij_tp1_sil = self.shapelet_similarity(
                S_tp1
            )  # update SS_tp1
            ...
            part3 = (
                0.5
                * self.lambda_2
                * np.trace((W_tp1.T @ X_tp1 - GY_tp1).T @ (W_tp1.T @ X_tp1 - GY_tp1))
            )
            ...
            S_tp1 = self.update_S(
                GY_tp1, X_tp1, W_tp1, G_tp1, S_tp1, Xkj_tp1_skl, SSij_tp1_sil, SS_tp1
            )
    ...
    def _predict(self, X, y=None) -> np.ndarray:
        ...
        segment_matrix = self._initiliasation_s(X)
        distance_matrix, _ = self._distance_timeseries_shapelet(X, segment_matrix)
        _, distance_matrix_to_centres = self._ussl_kmeans_assign(
            distance_matrix, self.cluster_centers_
        )
        GY_tp1 = distance_matrix_to_centres
    ...
    def derivation_of_S(self, Y, X, W, ST_t, Shape_t, Xkj_skl, SSij_sil, SS):
        ...
        for k in range(mDShape):
            ...
            for l in range(len_k):  # noqa: E741
                for i in range(mST):
                    for j in range(nST):
                        STij_skl[i, j, k, l] = (
                            ST_t[i, j]
                ...
                Part1[k, l] = np.sum(P1)
                Part2[k, l] = (
                    2 * np.sum(parameter2[k, :] * SSij_sil[k, :, l])
                    - parameter2[k, k] * SSij_sil[k, k, l]
                )
        mY, nY = Y.shape
    ...
    def shapelet_similarity(self, S):
        ...
        for i in range(mS):
            ...
            for j in range(i, mS):
                ...
                XS[i, j], XSij_si = self._distance_longseries_shortseries(
                    sh_l.reshape(1, -1), sh_s
                )  # calculate distance
                ...
                Hij_sil[i, j, :length_s] = (
                    H[i, j] * (-2 / self.sigma**2 * XS[i, j]) * XSij_si
                )
    ...
    def spectral_timeseries_similarity(self, X):
        ...
        for j in range(n_timepoints):
            for h in range(j, n_timepoints):
                ...
                G[j, h] = np.exp(
                    -(g**2) / self.sigma**2
                )  # Similarity between the j-th time series and the h-th time series
                G[h, j] = G[j, h]
            D_G[j, j] = np.sum(G[j, :])
    ...
    def _initiliasation_s(self, X: np.ndarray):
        S_0 = np.zeros(
            (
                self.n_equal_length_shapelets * self.n_scales_of_shapelet_length,
        ...
        for j in range(1, self.n_scales_of_shapelet_length + 1):
            ...
            S = np.ones((self.n_equal_length_shapelets, 1)) * np.mean(
                segment_matrix, axis=0
            )
    ...
    def _distance_timeseries_shapelet(self, X, segment_matrix):
        ...
        for j in range(n_cases):
            for k in range(segment_cases):
                ...
                distance, dist_derivative = self._distance_longseries_shortseries(
                    X[j], shapelet
                )
    ...
    def _distance_longseries_shortseries(self, ts, candidate_shapelet):
        ...
        )  # the distance between the series_long and series_short
        ...
        for candidate_idx in range(candidate_length):
            ...
            exp_alpha_D2_term = exp_alpha_D2 * (
                dist_lower * (1 + self.alpha * dist_2) - self.alpha * dist_upper
            )
    ...
    def _ussl_weird_kmeans(self, X):
        ...
        while v > epsilon:
            ...
            v = np.trace(
                (new_centers - cluster_centers).T @ (new_centers - cluster_centers)
            )
...
#     print(f"Number of clusters: {n_clusters}")  # noqa: T001
...
#     print(f"Train Labels: {temp}")  # noqa: T001
#     print(f"Train score: {train_score}")  # noqa: T001
#
#     if COMBINE:
